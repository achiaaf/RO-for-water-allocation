...
# d_set = (d_uncertain >= -0.2 * standard_dev + demand_mean, d_uncertain <= 0.2 * standard_dev + demand_mean)
d_set = (d_uncertain >= -1 * standard_dev, d_uncertain <= 1 * standard_dev)
# c_set =
# Define the objective function
prob.maxmin((prices - expenses) @ land - ((cost @ q).sum() + (cost @ d).sum()), d_set)
...
# capacity[5] = 0.7 * demand_mean + d_uncertain
...
# df.to_excel('RO Uncertainty.xlsx', sheet_name='Water Allocated')
# d3.to_excel('Land Uncertainty.xlsx')
